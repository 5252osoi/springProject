...
<div class="container">
  ...
  <div>
  	<form method="post">
	  	...
	  	<input type="hidden" name="job" id="job" value="학생"/>
	  	<!-- lombok을 사용할 때 2번째 글자만 대문자면 오류가 발생한다.. -->
	  	<input type="hidden" name="WDate" id="WDate" value="<%=new java.util.Date() %>"/>
	  	<div><input type="submit" value="제출" class="btn btn-success mt-2 form-control"/> </div>
  	</form> 
  </div>
  <hr/>
    <div>
  	<c:if test="${!empty vo}">넘어온 값 : ${vo.name} / ${vo.age} / ${vo.mid} / ${vo.pwd} / ${vo.gender} / ${vo.address} /${vo.job} /${vo.WDate}</c:if><br/>
  </div>
  <hr/>
  <p>
